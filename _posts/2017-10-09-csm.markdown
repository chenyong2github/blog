...
1.主相机整个视锥生成一张阴影图，会导致单个物体占用阴影图的比例太小，读取阴影图时出现采样精度不够（多个像素采样一个图素），产生锯齿。<br />
2.相机近端和远端物体对阴影图采样精度一样，会导致近端物体采样精度不够，远端物体采样精度浪费。<br />
使用尺寸更大的阴影图可以解决这个问题，但会导致内存使用增加。
...
#### 灯光相机视锥
正交的灯光相机视锥为长方体，为了减少灯光相机的无效绘制，需调整大小为主相机视锥的包围盒，见上面3级视锥分级示意图。<br />

下面先介绍对主相机的整个视锥如何求包围盒：
...
因为计算得到的包围盒会被作为灯光相机的视锥，所以一般包围盒的计算转换到灯光相机空间进行。
```
...
Unity底层实现是用一张2x2阴影图集来保存4级阴影图。由于我使用的Unity版本原因，这里使用4张单独的rt来分别保存。Unity2017新版本里已经加入对RenderTexture分区域更新的特性，这样也可以在上层实现类似底层的级联阴影图集。
...
CSM能有效解决透视相机下的阴影锯齿问题，基本的SM通过增大阴影图大小可改善阴影质量，但不能根本解决问题。
